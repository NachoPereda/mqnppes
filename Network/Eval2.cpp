...
int Eval2::quadraticEvaluation()
{
    ...
    for (int fil = 0; fil < mask; fil++)
    {
        ...
        {
            result = result + pictures[7].getMatrixElement(fil, j).second * pictures[1].getMatrixElement(fil, j + 1).second * pictures[1].getMatrixElement(fil, j + 2).second;
            std::cout << "Evaluando:  " <<pictures[7].getMatrixElement(fil, j).first << pictures[1].getMatrixElement(fil, j + 1).first << pictures[1].getMatrixElement(fil, j + 2).first << std::endl;
